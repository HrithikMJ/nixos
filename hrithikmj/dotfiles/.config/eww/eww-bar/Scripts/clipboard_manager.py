...
clipboard_history = (
    subprocess.run("cliphist list", shell=True, capture_output=True)
    .stdout.decode("utf-8")
    .strip()
    .split("\n")
)
...
for index, item in enumerate(clipboard_history):
    try:
        clipboard_history[index] = item.split("\t")[1]
    except:
        pass
...
buttons = [
    f'(button :class "clipboard-item" :onclick "wl-copy \\"{item}\\";eww --config ~/nixos/hrithikmj/dotfiles/.config/eww/eww-bar/ close clipboard_manager" "{shortened_text[index]}")'
    for index, item in enumerate(clipboard_history)
]
...
subprocess.run(
    f'echo \'(box :spacing "2" :orientation "vertical" :class "clipboard_container" {result_string})\'',
    shell=True,
)
